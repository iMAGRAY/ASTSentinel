...
fn benchmark_truncate_for_display(c: &mut Criterion) {
    ...
    for (name, input) in test_strings {
        group.bench_with_input(
            BenchmarkId::new("50_chars", name),
            &input,
            |b, s| b.iter(|| truncate_for_display(black_box(s), 50))
        );
    }
...
fn benchmark_large_file_processing(c: &mut Criterion) {
    ...
    for (name, size) in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("truncate", name),
            &content,
            |b, s| b.iter(|| truncate_for_display(black_box(s), 100))
        );
    }
