...
fn with_env<K: AsRef<str>, V: AsRef<str>, F: FnOnce()>(pairs: &[(K, V)], f: F) {
    let saved: Vec<(String, Option<String>)> = pairs
        ...
        .collect();
    for (k, v) in pairs.iter() { std::env::set_var(k.as_ref(), v.as_ref()); }
    f();
    for (k, v) in saved { match v { Some(val) => std::env::set_var(k, val), None => std::env::remove_var(k) } }
}
...
fn config_loads_from_env_and_json() {
    ...
    with_env(&[("SENSITIVITY","low"), ("AST_ENV","production"), ("AST_ALLOWLIST_VARS","example_,sample_"), ("AST_IGNORE_GLOBS","**/mocks/**,**/fixtures/**"), ("HOOKS_CONFIG_FILE", cfg_file.to_string_lossy().as_ref())], || {
...
fn is_test_context_matches_paths() {
    ...
    cfg.env = config::Environment::Production;
