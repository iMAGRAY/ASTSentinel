...
fn test_real_project_scanning() {
    ...
    // Scan the project
    let structure = scan_project_structure(current_dir_str, Some(config))
    ...
    let has_target_files = structure.files
        .iter()
    ...
    let has_git_files = structure.files
        .iter()
    ...
    let rust_files: Vec<_> = structure.files
        .iter()
...
fn test_gitignore_respected() {
    ...
    // Scan
    let structure = scan_project_structure(root.to_str().unwrap(), None)
    ...
    let file_names: Vec<_> = structure.files
        .iter()
