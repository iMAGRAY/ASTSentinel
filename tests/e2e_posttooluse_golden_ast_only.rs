...
#[test]
fn e2e_posttooluse_golden_sections_order_ast_only() {
    let temp = tempfile::tempdir().unwrap();
    let dir = temp.path();
    let file_path = dir.join("golden_ast.py");

    let old_code = "def f(a,b):\n    return a+b\n";
    let new_code = "def f(a):\n    return a\n"; // shrink params to exercise API CONTRACT in AST_ONLY
    std::fs::write(&file_path, new_code).unwrap();

    let hook_input = serde_json::json!({
        "tool_name": "MultiEdit",
        "tool_input": {"file_path": file_path.to_string_lossy(), "edits": [{"old_string": old_code, "new_string": new_code}]},
