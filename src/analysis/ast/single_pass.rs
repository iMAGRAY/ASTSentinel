...
impl SinglePassEngine {
    pub fn analyze(
        ...
        message: format!("Line exceeds 120 characters ({len})", len = line.len()),
                    ...
                    line: i + 1,
                    column: 121,
                    rule_id: "LINE001".to_string(),
                    points_deducted: ((line.len() - 120) / 10) as u32 + 1,
                });
            }
        }
            ...
            if is_function_fast(language, &kind_ids, &node) {
                ...
                if params > 5 {
                    issues.push(ConcreteIssue {
                        ...
                        message: format!("Function has too many parameters ({params} > 5)"),
                        file: String::new(),
                        line: node.start_position().row + 1,
                        column: node.start_position().column + 1,
                        rule_id: "PARAMS001".to_string(),
                        points_deducted: 15,
                    });
                }
                let (complexity, max_depth) = scan_complexity(language, &node);
                let threshold = match language {
                    SupportedLanguage::Python => 8,
                    SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => 10,
                    SupportedLanguage::Java | SupportedLanguage::CSharp => 12,
                    SupportedLanguage::Go => 8,
                    _ => 10,
                };
                if complexity > threshold {
                    issues.push(ConcreteIssue {
                        ...
                        line: node.start_position().row + 1,
                        column: node.start_position().column + 1,
                        rule_id: "AST003".to_string(),
                        points_deducted: (complexity - threshold) * 5,
                    });
                }
                if max_depth > 4 {
                    issues.push(ConcreteIssue {
                        ...
                        line: node.start_position().row + 1,
                        column: node.start_position().column + 1,
                        rule_id: "NEST001".to_string(),
                        points_deducted: (max_depth - 4) * 10,
                    });
                }
            }
                ...
                if let Some(next) = node.next_sibling() {
                    ...
                    if !skip {
                        issues.push(ConcreteIssue {
                            ...
                            message: if is_py_raise {
                                "Unreachable code after raise statement".to_string()
                            } else if is_py_loop_term {
                                "Unreachable code after loop control (break/continue)".to_string()
                            } else {
            ...
            match language {
                ...
                SupportedLanguage::CSharp | SupportedLanguage::Go => {
                    let is_string = match (language, kind) {
                        (SupportedLanguage::CSharp, k) => {
                            k == "string_literal"
                                || k.contains("interpolated_string")
                                || k.contains("verbatim_string")
                        }
                        (SupportedLanguage::Go, k) => {
                        ...
                        if let Ok(text) = node.utf8_text(src_bytes) {
                                ...
                                && likely_assignment_context(language, &node)
                            {
                                issues.push(ConcreteIssue {
                                    severity: IssueSeverity::Critical,
                                    category: IssueCategory::HardcodedCredentials,
                                    message: "Hardcoded credentials in string literal".to_string(),
                                    file: String::new(),
                                    line: node.start_position().row + 1,
                                    column: node.start_position().column + 1,
                                    rule_id: "SEC001".to_string(),
                                ...
                                || (text.contains("DELETE") && text.contains("FROM"))
                            {
                                issues.push(ConcreteIssue {
                                    severity: IssueSeverity::Major,
                                    category: IssueCategory::SqlInjection,
                                    message: "Possible SQL in string literal — validate parameterization"
                                        .to_string(),
                                    file: String::new(),
                                    line: node.start_position().row + 1,
                                    column: node.start_position().column + 1,
                ...
                SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => {
                    ...
                    if likely_assignment_context(language, &node) {
                        ...
                        while up < 4 {
                            ...
                            let k = cur.kind();
                            if k == "variable_declarator" || k == "assignment_expression" {
                                found = Some(cur);
                                break;
                            }
                        }
                        if let Some(ctx) = found {
                            ...
                            if let Some(left) = ctx.child(0) {
                                if let Ok(name) = left.utf8_text(src_bytes) {
                                    ...
                                    if lname.contains("password")
                                        || lname.contains("secret")
                                        || lname.contains("api_key")
...
"#;
        ...
        assert!(issues
            .into_iter()
            .all(|i| !matches!(i.category, IssueCategory::UnreachableCode)));
    }
    ...
    fn ts_too_many_params_detected() {
        let code =
            "function f(a?: number,b?: number,c?: number,d?: number,e?: number,f?: number){ return 1 }";
    ...
    let check = |n: &tree_sitter::Node| -> bool {
        ...
        match lang {
            SupportedLanguage::CSharp => {
                k.contains("assignment") || k.contains("declarator") || k.contains("variable_declaration")
            }
            SupportedLanguage::Go => {
                k.contains("assignment") || k.contains("declaration") || k.contains("short_var_declaration")
            }
            SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => {
...
fn is_function_fast(
    ...
    if let Some(ids) = ids {
        ...
        let by_id = match lang {
            SupportedLanguage::Python => {
                k == ids.py_function_definition || k == ids.py_async_function_definition
            }
            SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => {
                k == ids.js_function_declaration
    ...
    if cur.goto_first_child() {
        loop {
            ...
            let is_list = match lang {
                ...
                SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => {
                    matches!(n.kind(), "formal_parameters" | "parameters")
                }
            ...
            if is_list {
                return count_param_nodes(lang, &n);
            }
        ...
        loop {
            ...
            let is_param = match lang {
                SupportedLanguage::Python => matches!(
                    k,
                    "identifier" | "typed_parameter" | "default_parameter" | "list_splat_pattern"
                ),
                SupportedLanguage::JavaScript | SupportedLanguage::TypeScript => matches!(
                    k,
                    // JS params
                ...
                SupportedLanguage::Java => {
                    matches!(k, "formal_parameter" | "receiver_parameter" | "spread_parameter")
                }
                SupportedLanguage::CSharp => matches!(k, "parameter" | "parameter_array"),
                SupportedLanguage::Go => {
                    matches!(k, "parameter_declaration" | "variadic_parameter_declaration")
                }
                _ => false,
            };
            if is_param {
