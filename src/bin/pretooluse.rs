        ...
        if let Some(transcript_path) = &hook_input.transcript_path {
            ...
            match read_transcript_summary(transcript_path, 15, 1500) {
                Ok(summary) => {
                    writeln!(
                        log_file,
                        "Transcript content (last 15 msgs, max 1500 chars):"
                    )
                    .ok();
    ...
    match perform_validation(&config, &content, &hook_input).await {
        Ok(validation) => {
            let (decision, reason) = match validation.decision.as_str() {
                ...
                unknown => {
                    eprintln!(
                        "Warning: Unknown validation decision '{}', defaulting to deny for safety",
                        unknown
                    );
...
fn format_code_as_diff(hook_input: &HookInput) -> String {
    ...
    match hook_input.tool_name.as_str() {
        "Edit" => {
            ...
            if let Some(old_string) = hook_input
                .tool_input
                .get("old_string")
                .and_then(|v| v.as_str())
            {
                if let Some(new_string) = hook_input
                    .tool_input
                    .get("new_string")
                    .and_then(|v| v.as_str())
                {
        ...
        "Write" => {
            ...
            if let Some(content) = hook_input
                .tool_input
                .get("content")
                .and_then(|v| v.as_str())
            {
        ...
        _ => {
            ...
            let content =
                extract_content_from_tool_input(&hook_input.tool_name, &hook_input.tool_input);
            if !content.is_empty() {
                diff = format!("Content:\n{}", content);
            }
        }
