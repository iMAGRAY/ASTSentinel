...
use rust_validation_hooks::{Config, sanitize_zero_width_chars, truncate_utf8_safe};
...
mod config {
    ...
    fn parse_max_stdin_size() -> usize {
        match env::var("MEMORY_MAX_STDIN_SIZE") {
            Ok(val) => {
                ...
                if trimmed.is_empty() {
                    log_warning(&format!("MEMORY_MAX_STDIN_SIZE is empty or whitespace, using default {}", 
                ...
                match trimmed.parse::<usize>() {
                    ...
                    Err(_) => {
                        log_warning(&format!("Invalid MEMORY_MAX_STDIN_SIZE='{}', using default {}", 
    ...
    fn validate_stdin_size(size: usize) -> usize {
        if size >= MIN_STDIN_SIZE && size <= MAX_STDIN_SIZE {
            ...
            log_warning(&format!("MEMORY_MAX_STDIN_SIZE={} is outside valid range ({}-{}), using default {}", 
...
fn construct_project_claude_md_path(hook_input: &StopEventInput) -> Result<String> {
    if let Some(cwd) = &hook_input.cwd {
        ...
        if cwd.chars().count() > MAX_PATH_LENGTH {
            anyhow::bail!("Path too long: {} chars > {} limit", cwd.chars().count(), MAX_PATH_LENGTH);
        }
...
fn json_to_claude_xml(json_value: serde_json::Value) -> Result<String> {
    ...
    if !json_value.is_object() {
        anyhow::bail!("Expected JSON object for memory structure, got {}", json_value.to_string());
    }
...
async fn write_xml_memory_file(memory_file: &std::path::PathBuf, xml_content: &str) -> Result<()> {
    ...
    let canonical_path = validate_secure_path(memory_file)
        .with_context(|| "Path security validation failed")?;
    
    // Validate XML with proper parser before writing
    validate_xml_with_parser(xml_content)
        .with_context(|| "XML content validation failed")?;
    ...
    // Write to temporary file first
    {
        ...
        writer.write_all(xml_content.as_bytes()).await
    ...
    tokio::fs::rename(&temp_file, &canonical_path).await
...
fn parse_xml_size_value(val: &str) -> usize {
    ...
    if trimmed.is_empty() {
        eprintln!("Warning: MEMORY_MAX_XML_SIZE is empty, using default {}", xml_config::DEFAULT_MAX_XML_SIZE);
    ...
    match trimmed.parse::<i64>() {
        Ok(size) if size < 0 => {
            eprintln!("Warning: MEMORY_MAX_XML_SIZE ({}) is negative, using default {}", size, xml_config::DEFAULT_MAX_XML_SIZE);
        ...
        Ok(size) => {
            ...
            if size_usize < xml_config::MIN_XML_SIZE {
                eprintln!("Warning: MEMORY_MAX_XML_SIZE ({}) is too small, minimum is {}, using default", 
                    size_usize, xml_config::MIN_XML_SIZE);
                xml_config::DEFAULT_MAX_XML_SIZE
            } else {
                size_usize
            }
...
fn validate_xml_size(xml_content: &str) -> Result<()> {
    ...
    if content_size > max_xml_size {
        eprintln!("XML size validation failed: {} bytes exceeds limit of {} bytes", content_size, max_xml_size);
        anyhow::bail!("XML content size ({} bytes) exceeds maximum allowed size ({} bytes)", 
            content_size, max_xml_size);
    }
    
    if content_size < xml_config::MIN_XML_SIZE {
        eprintln!("XML size validation failed: {} bytes is below minimum of {} bytes", content_size, xml_config::MIN_XML_SIZE);
        anyhow::bail!("XML content size ({} bytes) is too small, minimum is {}", 
            content_size, xml_config::MIN_XML_SIZE);
    }
...
fn validate_secure_path(memory_file: &std::path::PathBuf) -> Result<std::path::PathBuf> {
    ...
    for component in memory_file.components() {
        match component {
            std::path::Component::ParentDir => {
                anyhow::bail!("Path contains '..' component, potential directory traversal: {:?}", memory_file);
            },
    ...
    let canonical_path = memory_file.canonicalize()
        .or_else(|_| {
            ...
            if let Some(parent) = memory_file.parent() {
                if let Some(filename) = memory_file.file_name() {
                    ...
                    Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid memory file path: no filename"))
                }
            } else {
                Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid memory file path: no parent directory"))
            }
        })
    ...
    if let Ok(metadata) = std::fs::symlink_metadata(&canonical_path) {
        if metadata.file_type().is_symlink() {
            ...
            eprintln!("Security Warning: Path points to a symlink: {:?}", canonical_path);
            anyhow::bail!("Path points to a symlink, potential security risk: {:?}", canonical_path);
        }
    }
        ...
        match memory_file.canonicalize() {
            Ok(memory_canonical) => {
                if canonical_path != memory_canonical {
                    ...
                    eprintln!("Security Info: Path was resolved through symlinks: {:?} -> {:?}", memory_file, canonical_path);
                }
            },
            Err(_) => {
                // If we can't canonicalize the original, just log this fact
                eprintln!("Security Info: Could not verify path normalization for: {:?}", memory_file);
            }
        }
    }
...
fn create_unique_temp_file(target_path: &std::path::PathBuf) -> Result<std::path::PathBuf> {
    ...
    let filename = target_path.file_stem()
    ...
    let temp_filename = format!("{}.{}.{}", filename, uuid.simple(), xml_config::TEMP_FILE_EXTENSION);
...
fn log_memory_operation_success(memory_file: &std::path::PathBuf, operation_type: &str) {
    if std::env::var("DEBUG_HOOKS").unwrap_or_default() == "true" {
        ...
        match memory_file.file_name().and_then(|name| name.to_str()) {
            Some(filename) => {
                eprintln!("Memory organizer: Successfully saved {} to {}", operation_type, filename);
            }
            None => {
                eprintln!("Memory organizer: Successfully saved {} (filename not UTF-8)", operation_type);
            }
        }
    } else {
...
fn format_current_session(session: &serde_json::Value, indent: usize) -> Result<String> {
    ...
    if let Some(active_context) = session.get("active_context") {
        ...
        if let Some(task) = active_context.get("current_task") {
            xml.push_str(&format!("{}      <current-task>{}</current-task>\n", 
                indent_str, xml_escape(task.as_str().unwrap_or(""))));
        }
        
        if let Some(action) = active_context.get("last_action") {
            xml.push_str(&format!("{}      <last-action>{}</last-action>\n", 
                indent_str, xml_escape(action.as_str().unwrap_or(""))));
        }
            ...
            if let Some(array) = steps.as_array() {
                for step in array {
                    xml.push_str(&format!("{}        <step>{}</step>\n", 
                        indent_str, xml_escape(step.as_str().unwrap_or(""))));
                }
            }
            xml.push_str(&format!("{}      </next-steps>\n", indent_str));
        }
        ...
        if let Some(status) = active_context.get("status") {
            xml.push_str(&format!("{}      <status>{}</status>\n", 
                indent_str, xml_escape(status.as_str().unwrap_or(""))));
        }
            ...
            if let Some(array) = files.as_array() {
                for file in array {
                    xml.push_str(&format!("{}        <file>{}</file>\n", 
                        indent_str, xml_escape(file.as_str().unwrap_or(""))));
                }
            }
            xml.push_str(&format!("{}      </working-files>\n", indent_str));
        }
    ...
    if let Some(insights) = session.get("key_insights") {
        ...
        if let Some(array) = insights.as_array() {
            for insight in array {
                xml.push_str(&format!("{}      <insight>{}</insight>\n", 
                    indent_str, xml_escape(insight.as_str().unwrap_or(""))));
            }
        }
        xml.push_str(&format!("{}    </key-insights>\n", indent_str));
    }
            ...
            for item in array {
                ...
                if let Some(content) = item.get("content") {
                    xml.push_str(&format!("{}        <content>{}</content>\n", 
                        indent_str, xml_escape(content.as_str().unwrap_or(""))));
                }
                if let Some(location) = item.get("location") {
                    xml.push_str(&format!("{}        <location>{}</location>\n", 
                        indent_str, xml_escape(location.as_str().unwrap_or(""))));
                }
                ...
                if let Some(type_val) = item.get("type") {
                    xml.push_str(&format!("{}        <type>{}</type>\n", 
                        indent_str, xml_escape(type_val.as_str().unwrap_or(""))));
                }
...
fn format_metadata(metadata: &serde_json::Value, indent: usize) -> Result<String> {
    ...
    if let Some(updated) = metadata.get("last_updated") {
        xml.push_str(&format!("{}    <last-updated>{}</last-updated>\n", 
            indent_str, xml_escape(updated.as_str().unwrap_or(""))));
    }
    ...
    if let Some(stats) = metadata.get("optimization_stats") {
        ...
        if let Some(count) = stats.get("memories_count") {
            xml.push_str(&format!("{}      <memories-count>{}</memories-count>\n", 
                indent_str, count.as_u64().unwrap_or(0)));
        }
        if let Some(ratio) = stats.get("reduction_ratio") {
            xml.push_str(&format!("{}      <reduction-ratio>{}</reduction-ratio>\n", 
                indent_str, ratio.as_f64().unwrap_or(0.0)));
        }
        if let Some(tokens) = stats.get("total_tokens") {
            xml.push_str(&format!("{}      <total-tokens>{}</total-tokens>\n", 
                indent_str, tokens.as_u64().unwrap_or(0)));
        }
    ...
    if let Some(source) = metadata.get("source") {
        xml.push_str(&format!("{}    <source>{}</source>\n", 
            indent_str, xml_escape(source.as_str().unwrap_or(""))));
    }
    ...
    if let Some(array) = solutions.as_array() {
        for solution in array {
            ...
            if let Some(problem) = solution.get("problem") {
                xml.push_str(&format!("{}      <problem>{}</problem>\n", 
                    indent_str, xml_escape(problem.as_str().unwrap_or(""))));
            }
            if let Some(sol_text) = solution.get("solution") {
                xml.push_str(&format!("{}      <solution-text>{}</solution-text>\n", 
                    indent_str, xml_escape(sol_text.as_str().unwrap_or(""))));
            }
...
fn format_project_info(project: &serde_json::Value) -> Result<String> {
    ...
    if let Some(created) = project.get("created_at") {
        xml.push_str(&format!("    <created-at>{}</created-at>\n", 
...
fn validate_gpt5_response(json_str: &str) -> Result<serde_json::Value> {
    let json_value: serde_json::Value = serde_json::from_str(json_str)
        .with_context(|| "Failed to parse GPT-5 JSON response")?;
...
fn log_token_metrics(metrics: &TokenMetrics) {
    eprintln!("Token analysis: Original: {}, Optimized (XML): {}, Reduced: {} ({:.1}%)", 
...
fn build_full_memory_structure(json_value: serde_json::Value) -> Result<serde_json::Value> {
    ...
    let _active_context = json_value.get("active_context")
        .ok_or_else(|| anyhow::anyhow!("Missing 'active_context' in GPT-5 response"))?;
    
    // Extract values with safe defaults
    let reduction_ratio = json_value.get("reduction_ratio")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.75);
    
    let total_tokens = json_value.get("total_tokens")
        .and_then(|v| v.as_u64())
        .unwrap_or(0) as usize;
    
    let solutions_archive = json_value.get("solutions_archive")
        .cloned()
    ...
    let memories_count = json_value.get("optimized_memories")
        .and_then(|v| v.as_array())
        .map(|arr| arr.len())
        .or_else(|| json_value.get("key_insights")
            .and_then(|v| v.as_array())
            .map(|arr| arr.len()))
        .unwrap_or(0);
...
Optimize and merge these memories, keeping only the most relevant information within the token limit.",
    ...
    if total_input_tokens > context_window {
        ...
        if total_input_tokens > context_window * 2 {
            anyhow::bail!("Input too large: {} tokens exceeds double the context window", total_input_tokens);
        }
    }
...
async fn read_and_validate_stdin_input() -> Result<StopEventInput> {
    ...
    let hook_input: StopEventInput = serde_json::from_str(&sanitized_buffer)
        .with_context(|| format!("Failed to parse input JSON. Input preview: {}", 
...
async fn main() -> Result<()> {
    ...
    let memory_file = match construct_project_claude_md_path(&hook_input) {
        Ok(path) => path,
        Err(e) => {
            eprintln!("Memory organizer: Failed to construct CLAUDE.md path: {}", e);
            println!(r#"{{"continue":true}}"#);  // Continue without blocking
            return Ok(());
        }
    };
    ...
    let optimization_xml = match process_memory_optimization(&transcript, &existing_memory, context_window).await {
        Ok(xml) => xml,
        Err(api_error) => {
