use rust_validation_hooks::{HookInput, PreToolUseOutput, PreToolUseHookOutput};
...
fn main() {
    ...
    let diff_output = match hook_input.tool_name.as_str() {
        "Edit" => {
            let file_path = hook_input.tool_input.get("file_path")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            let old_string = hook_input.tool_input.get("old_string")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let new_string = hook_input.tool_input.get("new_string")
                .and_then(|v| v.as_str())
                .unwrap_or("");
        ...
        "Write" => {
            let file_path = hook_input.tool_input.get("file_path")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            let content = hook_input.tool_input.get("content")
                .and_then(|v| v.as_str())
                .unwrap_or("");
        ...
        "MultiEdit" => {
            ...
            // Parse edits array
            let edits = hook_input.tool_input.get("edits")
    ...
    // Output the result
    println!("{}", serde_json::to_string(&output).unwrap_or_else(|_| "Error".to_string()));
}