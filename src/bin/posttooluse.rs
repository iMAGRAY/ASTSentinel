...
fn build_next_steps(score: &QualityScore) -> String {
    ...
    let mut sec = false; // security
    let mut corr = false; // correctness
    let mut maint = false; // maintainability
    let mut perf = false; // performance/complexity
    let mut style = false; // style/lint
    let mut docs = false; // documentation/tests
    ...
    for i in &score.concrete_issues {
        match i.category {
            ...
            | IssueCategory::HardcodedCredentials => sec = true,
            ...
            IssueCategory::LongLine | IssueCategory::UnusedImports => style = true,

            IssueCategory::MissingDocumentation => docs = true,
...
fn build_code_health(score: &QualityScore) -> String {
    ...
    s.push_str(&format!("Too many params: {params}\nDeep nesting: {nesting}\nHigh complexity: {complexity}\nLong methods: {long_method}\n"
    ));
    s
}
...
async fn build_change_summary(hook_input: &HookInput, display_path: &str) -> String {
    // Per product policy: do not expose raw diffs to user/agent.
    // Provide only a compact operation summary without code/diff.
    let mut s = String::new();
    ...
    // Minimal deterministic header without leaking content
    s.push_str(&format!("Tool: {} ({op})\n", hook_input.tool_name));
    if !display_path.is_empty() {
        s.push_str(&format!("File: {display_path}\n"));
    }
...
async fn perform_project_ast_analysis(working_dir: &str) -> String {
    ...
    if !results.is_empty() {
        ...
        for (path, issues_count, _) in results.iter().take(10) {
            analysis.push_str(&format!("- `{path}`: {issues_count} issues\n"));
        }
        if results.len() > 10 {
            analysis.push_str(&format!(
...
async fn main() -> Result<()> {
    ...
    let formatted_prompt = format_analysis_prompt_with_ast(
        ...
        Some(&diff_context), // Include diff for AI validation context
