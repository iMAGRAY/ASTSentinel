...
pub fn redact_with_report(input: &str) -> (String, usize) {
    ...
    let kv_patterns = [
        r"(?i)\b(password|pass|pwd)\b\s*[:=]\s*('?\"?)([^'"\s]+)\2",
        r"(?i)\b(api[_-]?key|apikey|secret|token|access[_-]?key|secret[_-]?access[_-]?key)\b\s*[:=]\s*('?\"?)([^'"\s]+)\2",
    ];
    ...
    let re_bearer = Regex::new(r"(?i)bearer\s+[A-Za-z0-9\-_.=]+") .unwrap();
    ...
    let re_conn = Regex::new(r"(?i)(jdbc:|postgres(?:ql)?:|mongodb:|redis:|amqp:|mongodb\+srv:)[^\s\n]+") .unwrap();
    ...
    let re_long = Regex::new(r"\b[A-Za-z0-9_\-]{32,}\b").unwrap();
    let mut local = 0usize;
    s = re_long
