...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Environment { Production, Test }

impl Default for Config {
    fn default() -> Self {
        Self {
            sensitivity: Sensitivity::Medium,
            ignore_globs: GlobSetBuilder::new().build().unwrap(),
...
pub fn load_config() -> Config {
    ...
    if let Ok(val) = std::env::var("AST_ALLOWLIST_VARS") {
        let list = val
            ...
            .filter(|s| !s.is_empty())
            .collect::<Vec<_>>();
        if !list.is_empty() { cfg.allowlist_vars = list; }
    }
    ...
    if let Ok(val) = std::env::var("AST_IGNORE_GLOBS") {
        ...
        for pat in val.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()) {
            if let Ok(glob) = Glob::new(pat) { builder.add(glob); }
        }
        if let Ok(set) = builder.build() { cfg.ignore_globs = set; }
    }

    // Optional JSON config file: path from HOOKS_CONFIG_FILE or .hooks-config.json in CWD
    let cfg_path = std::env::var("HOOKS_CONFIG_FILE").ok().unwrap_or_else(|| ".hooks-config.json".to_string());
    if let Ok(text) = std::fs::read_to_string(&cfg_path) {
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
            ...
            if let Some(list) = json.get("allowlist_vars").and_then(|v| v.as_array()) {
                ...
                for it in list { if let Some(s) = it.as_str() { vars.push(s.to_string()); } }
                if !vars.is_empty() { cfg.allowlist_vars = vars; }
            }
            if let Some(globs) = json.get("ignore_globs").and_then(|v| v.as_array()) {
                let mut builder = GlobSetBuilder::new();
                for it in globs { if let Some(pat) = it.as_str() { if let Ok(glob) = Glob::new(pat) { builder.add(glob); } } }
                if let Ok(set) = builder.build() { cfg.ignore_globs = set; }
            }
        }
    }
...
pub fn should_ignore_path(cfg: &Config, path: &str) -> bool {
    ...
    cfg.ignore_globs.is_match(p)
}

pub fn is_test_context(cfg: &Config, path: &str) -> bool {
    if matches!(cfg.env, Environment::Test) { return true; }
    let lower = path.replace('\\', "/").to_ascii_lowercase();
    lower.contains("/test/") || lower.contains("/tests/") || lower.contains("__tests__") || lower.contains("/spec/") || lower.ends_with("_test.rs") || lower.ends_with(".spec.ts")
}
