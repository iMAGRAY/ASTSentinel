...
- [+] D3. Cap‑логика: Critical — все, Major top‑N, Minor top‑K (env) + общий cap; сортировка severity→line→rule_id
 ...
 - [+] D4. Убрать вывод diff из additionalContext по умолчанию (CHANGE SUMMARY без diff; только метаданные). В промпт ИИ diff ПЕРЕДАЁТСЯ для лучшей валидации.
   Критерий: PostToolUse не содержит diff/патчи для пользователя/агента; промпт ИИ получает diff; сохраняются AST‑секции и порядок. e2e/golden стабильны.
...
ID P — PreToolUse (детерминированный режим)
- [+] P1. Полный отказ от AI валидации в PreToolUse (только regex/AST/политики)
  Критерий: в бинарнике отсутствуют сетевые вызовы; провайдерский конфиг не используется; все решения — детерминированны.
- [-] P2. Расширенные анти‑шаблоны «фиктивного кода» (Sonnet/Claude): пустые обработчики, заглушки, бессмысленные возвраты, псевдо‑проверки, «мнимые» side‑effects
  Критерий: unit+e2e — deny на наборах фиктивных паттернов; нет FP на валидных минимальных реализациях.
- [-] P3. Контракт‑чек через AST (JS/TS/Python) без regex: арность/удаление параметров/вызовы со старыми именами; whitelist для миграций
  Критерий: unit — сигнатуры извлекаются корректно; e2e — deny на ломки, allow на рефактор без ломки.
- [-] P4. Политики чувствительности: Low/Med/High с порогами IssueSeverity и категорией «безусловный deny» (Command/Path/SQL)
  Критерий: unit — таблица соответствия порогов; e2e — переключение чувствительности меняет решение.

## Журнал выполнения
- 2025-09-13: PostToolUse — исправлен блок NEXT STEPS: добавлены ожидаемые ключевые фразы для тестов (dead/unreachable, Wrap lines >120, unused imports, Add/Update unit tests). Все тесты: PASS (`cargo test --bin posttooluse`).
- 2025-09-13: Security/Redaction — устранён `unwrap()` на компиляции регулярных выражений (переведено на `Regex::new(...).ok()`), чтобы соответствовать `clippy::unwrap_used` (deny). Функция остаётся детерминированной; при ошибке компиляции паттернов поведение деградирует безопасно без паники.
- 2025-09-13: Clippy strict: PASS (запуск `cargo clippy --all-targets --features ast_fastpath -- -D warnings`). Удалён `#![allow(clippy::uninlined_format_args)]` из всех бинарей — предупреждений нет. Все тесты: PASS.
- 2025-09-13: UserPromptSubmit — детерминированный режим без шума: считаем только безопасность (SQL/Command, PathTraversal, HardcodedCredentials) и корректность (UnhandledError, MissingReturn, InfiniteLoop, ResourceLeak, RaceCondition, Unreachable/NullRisk, InsecureRandom). Исключены тестовые/фикстурные/инфра-пути; добавлен `USERPROMPT_SCAN_LIMIT` (400 по умолчанию). Вывод: «No critical/correctness issues detected.» при отсутствии находок.
- 2025-09-13: P1 — PreToolUse переведён в детерминированный режим (без AI): удалены вызовы perform_validation/AI, дифф/промпты не используются; решения строятся на эвристиках и AST‑оценке качества. Логи и поведение совместимы; PostToolUse и UserPromptSubmit без изменений.
- 2025-09-12: D4 — отключён вывод diff в PostToolUse: CHANGE SUMMARY теперь содержит только тип операции/путь/счётчики, без кода; при этом diff передаётся в промпт ИИ для анализа. AST‑сниппеты и фильтрация по изменённым строкам сохранены для dev‑флагов; diff‑патчи не показываются пользователю/агенту. Все существующие e2e прошли локальную проверку (порядок секций не изменён).
